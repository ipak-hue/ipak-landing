...
function ValueCard({ v, i }) {
  ...
  return (
    <div key={i} ref={ref} style={{
      ...
      transition: `opacity 0.8s cubic-bezier(0.22,1,0.36,1) ${i * 130}ms, transform 0.8s cubic-bezier(0.22,1,0.36,1) ${i * 130}ms`,
      cursor: 'default',
    }}
    onMouseEnter={e => { e.currentTarget.style.border = '1px solid rgba(243,106,34,0.4)'; e.currentTarget.style.background = 'rgba(243,106,34,0.06)'; e.currentTarget.style.transform = 'translateY(-6px) scale(1.02)' }}
    onMouseLeave={e => { e.currentTarget.style.border = '1px solid rgba(255,255,255,0.06)'; e.currentTarget.style.background = 'rgba(255,255,255,0.02)'; e.currentTarget.style.transform = 'translateY(0) scale(1)' }}
    >
      <div style={{ fontSize: '44px', marginBottom: '1.25rem' }}>{v.icon}</div>
...
export default function WhyIPak() {
  ...
  return (
    <section id="why-ipak" style={{ background: '#000000', padding: '6rem 0', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', bottom: '-100px', right: '-100px', width: '600px', height: '600px', background: 'radial-gradient(ellipse, rgba(243,106,34,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />
